...
/*   x Titre du produit (uniquement caractères alpha avec tiret minimum 5 caractères)
   ...
   X Type de vente: liste déroulante avec pour items "Neuf" , "Occasion", "Dematerielisé", "Autre". Quand je selection Autre (change())
   cela me fait apparaitre un champs en dessous
   x Boutons "Créer cette fiche produit"

   /*Bonus : Ajouter le champs "Heure de disponibilité" et verifier que le format soit XX:XX et que ce soit une heure valide (comprise entre 00 et 23 pour les heures et 00 à 60 minutes)
...
//+ Description: 10 mots avec des caractres alpha numérique avec espace et baslises HTML <b>
     ...
     var textInput = $('#summernote').code();
     var nbDeMot = textInput.split(" ");
     var regexText = /^[a-z0-9\-\ ]?.{1,}$/im;


     console.log(textInput);

     if(regexText.test(textInput) && nbDeMot.length>10){

     textInput.css('color','green')
     }
     else{
